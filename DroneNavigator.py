...
class DroneNavigator:
    ...
    def generate_path(self) -> List[Point]:
        ...
        while not path_clear and iteration < self.max_iterations:
            ...
            middle_point, zone_details = ObstacleAvoidance.find_first_red_zone_point(path, self.red_zones)
            
            print(f"Middle red zone point: {middle_point.lat,middle_point.lon}\nZone details: {zone_details.radius,zone_details.center.lat,zone_details.center.lon}")
            ...
            if middle_points_list:
                point_right, point_left = self.path_planner.get_points_around_middle_point(zone_details, middle_points_list[-1], current_bearing)
            else:
            ...
            for point in [preferred_point, alternative_point, preferred_point]:
                path, last_middle_point_index = self.path_planner.generate_complete_path_updated(self.start, [point] + middle_points_list, self.goal)
                path_clear = ObstacleAvoidance.path_is_clear_of_red_zones(path, self.red_zones)
                if path_clear:
                    break
